...
export default function PaymentVerifyClient() {
  ...
  const verifyPaymentWithId = async (idToVerify) => {
    try {
      ...
      if (result?.success) {
        ...
        if (result.order) {
          handleSMSAIntegration(result.order).catch(err => {
            console.error("SMSA integration error (non-blocking):", err);
          });
        }
      } else {
        setStatus("failed");
  ...
  const handleSMSAIntegration = async (order) => {
    try {
      ...
      // Get country code from localStorage (customerData)
      const countryCode = customerData?.country_code || "SA";

      // Check if order qualifies for SMSA Express - only for SA
      ...
      const customerPhone = customerData?.phone || order.user?.phone || "";
      if (!customerPhone || customerPhone.trim() === "") {
        ...
        setSmsaData({ 
          success: false, 
          error: "Customer phone number is required for shipping" 
        });
        return;
      }
      ...
      const shipmentData = {
        ...
        customerPhone: customerPhone, // Use validated phone
        ...
        items: order.items || [],
        totalAmount: order.total_amount || 0, // Add total amount from order
      };

      console.log("📋 Shipment Data Prepared:", {
        hasPhone: !!shipmentData.customerPhone,
        phone: shipmentData.customerPhone.substring(0, 3) + "***", // Mask for logging
        hasName: !!shipmentData.customerName,
        hasAddress: !!shipmentData.shippingAddress.address_line_1,
        hasCity: !!shipmentData.shippingAddress.locality,
      });

      // Call SMSA Express API
      console.log("📤 Sending SMSA shipment request:", {
        orderId: shipmentData.orderId,
        countryCode: shipmentData.shippingAddress.country_code,
        hasAddress: !!shipmentData.shippingAddress.address_line_1,
        hasCity: !!shipmentData.shippingAddress.locality,
      ...
      if (result.success) {
        ...
        console.error("❌ SMSA shipment failed:", {
          error: errorMessage,
