...
export async function POST(request) {
  try {
    ...
    // Use rawRequest to get both data and errors (GraphQL can return partial data with errors)
    try {
      const { data, errors } = await client.rawRequest(query, variables || {});

      // Return data even if there are errors (GraphQL allows partial data)
      // The client will handle filtering problematic data
      return NextResponse.json({ 
        data: data || null,
        errors: errors || null,
        error: errors && errors.length > 0 ? errors.map(e => e.message || e.debugMessage || "GraphQL error").join("; ") : null
      });
    } catch (rawError) {
      // rawRequest might still throw in some cases, check if it has response data
      if (rawError.response) {
        const { data, errors } = rawError.response;
        return NextResponse.json({ 
          data: data || null,
          errors: errors || null,
          error: errors && errors.length > 0 ? errors.map(e => e.message || e.debugMessage || "GraphQL error").join("; ") : null
      ...
      // Re-throw if it's not a GraphQL response error
      throw rawError;
    }
  } catch (error) {
    ...
    // Check if we have partial data despite the error
