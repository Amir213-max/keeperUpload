...
export const PRODUCT_QUERY = gql`
  query Product($id: ID!) {
    product(id: $id) {
      ...
      variants {
        id
        # FIX: Removed name field as it can be null and causes GraphQL error "Cannot return null for non-nullable field ProductVariant.name"
        # name
        price
      }
      brand {
...
// Fallback query without variants for products with problematic variants
export const GET_PRODUCT_BY_SKU_NO_VARIANTS = gql`
  query GetProductBySkuNoVariants($sku: String!) {
    productBySku(sku: $sku) {
      id
      name
      sku
      ...
      description_en
      images
      # FIX: Removed variants entirely as fallback
      productAttributeValues {
        id
        key
        attribute {
