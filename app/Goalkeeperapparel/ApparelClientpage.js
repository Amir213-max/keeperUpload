...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Parse path segments (everything after /Goalkeeperapparel)
    const pathParts = currentPath.split("/").filter((p) => p);
    const basePathParts = "/Goalkeeperapparel".split("/").filter((p) => p);
  ...
  useEffect(() => {
    ...
    // 🔹 تفعيل التحميل عند تغيير الفلاتر
    setIsLoading(true);
    setShowProducts(false);
    
    // محاكاة التحميل ثم عرض المنتجات
    setTimeout(() => {
      setFilteredProducts(result);
      setCurrentPage(1);
      setIsLoading(false);
      // عرض المنتجات بإنيميشن بعد اكتمال التحميل
      setTimeout(() => {
  ...
  return (
    <div className={`bg-[#373e3e] ${isRTL ? "rtl" : "ltr"}`}>
      {/* ProgressBar - شريط التحميل الأصفر */}
      <ProgressBar 
        isLoading={isLoading} 
        onComplete={() => {
          // يمكن إضافة أي منطق إضافي هنا عند اكتمال التحميل
        }}
      />
            ...
            onSelectCategory={(catId) => {
              ...
              if (selectedCat) {
                if (catId === selectedCategoryId) {
                  ...
                  router.replace('/Goalkeeperapparel', { scroll: false });
                } else {
  ...
  {currentProducts && currentProducts.length > 0 ? currentProducts.map((product, index) => {
    ...
    <div className="col-span-full text-center py-12">
      ...
      <p className="text-gray-400 text-sm mt-2">{t("Please try again later")}</p>
    </div>
  )}
</div>
