...
export default function Footer() {
  ...
  useEffect(() => {
   ...
   method: 'POST',
        ...
        // استبعاد offers_label من العرض في Footer
  ...
  return (
    <footer className="bg-neutral-900 text-white text-sm">
      ...
      <div className="bg-yellow-400 text-black px-4 py-6 flex flex-col sm:flex-row items-center justify-center gap-4">
        ...
        />
        <button className="bg-white px-6 py-2 rounded font-bold cursor-pointer hover:bg-gray-200 w-full sm:w-auto" aria-label="Subscribe to newsletter">
          SIGN UP!
        </button>
      </div>
      ...
      <ul className="space-y-4 space-x-2">
        {values.map((setting, i) => (
          <li key={i} className="cursor-pointer hover:text-amber-400 transition">
            {setting.url ? (
              <Link href={setting.url} aria-label={setting.value}>{setting.value}</Link>
            ) : (
