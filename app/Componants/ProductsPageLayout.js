...
// Currency handling is now managed by CurrencyContext
  ...
  useEffect(() => {
    ...
    Object.entries(selectedAttributes).forEach(([attr, values]) => {
      if (values && values.length > 0) params.set(`attr_${attr}`, values.join(","));
    });

    // 🔹 إذا لم يكن هناك فلاتر، استخدم pathname فقط بدون query parameters
    const queryString = params.toString();
    const newUrl = queryString ? `${pathname}?${queryString}` : pathname;
    
    router.replace(newUrl, { scroll: false });
  }, [selectedBrand, selectedCategoryId, selectedAttributes, router, pathname]);
    ...
    <div className={`bg-[#373e3e] min-h-screen ${isRTL ? "rtl" : "ltr"}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-0">
        ...
        <div className="lg:col-span-4 bg-white p-4 sm:p-6  ">
          ...
          {brands.length > 1 && !selectedCategoryId && !selectedBrand && (
            <BrandsSlider
              brands={brands}
